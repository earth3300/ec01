...
function format_hd_page( $post ){
	if ( ! empty( $post ) ){
		$str = '';
		$page = get_hd_page_data();
		$args = get_hd_page_args( $post );
		$str .= $page['section'] ? sprintf( '<section id="page-%s" class="page">%s', $post -> ID, PHP_EOL ) : '';
		$str .= $page['inner'] ? '<div class="inner">' . PHP_EOL : '';
		$str .= $page['title'] ? sprintf( '<h4 class="title absolute top left negative-top-margin left-offset">%s</h4>', $post-> post_title ) : '';
		$str .= get_hd_html( $post );
		$str .= $page['author-bar'] && ! is_hd_video( $args['url'] ) ? get_hd_author_bar_html( $post, $author ) : '';
		$str .= $page['bottom-left'] ? get_hd_bottom_left_html( $post, $args ) : '';
		$str .= $page['bottom-right'] ? get_hd_bottom_right_html( $post, $args ) : '';
		$str .= $page['mp3'] ? get_hd_mp3( $args ) : '';
		$str .= $page['inner'] ? '</div>' . PHP_EOL : '';
		$str .= $page['page-number'] ? sprintf( '<div class="page-number">%s</div>%s', $post -> ID, PHP_EOL ) : '';
		$str .= $page['section'] ? '</section>' . PHP_EOL : '';
		return $str;
	}
}
...
function get_hd_page_args( $post ) {
	...
	if ( $post -> post_type == 'hd' ) {
		...
		$args['poster'] = get_the_post_thumbnail( $post->ID, 'full',  get_hd_page_classes( $args ) );
		if ( ! empty ( $args ) ) {
			return $args;
		}
		else {
...
function get_hd_page_classes( $args ){
	
	if ( isset( $args['layout'] ) && $args['layout'] == 'center' ) {
		$classes[] = 'center';
	}
	else if ( isset( $args['layout'] ) && $args['layout'] == 'left' ) {
		$classes[] = 'left';
	}
	else if ( isset( $args['layout'] ) && $args['layout'] == 'right' ) {
		$classes[] = 'right';
	}
	
	if ( isset( $args['overflow'] ) && $args['overflow'] == 'scroll' ) {
		$classes[] = 'scroll';
	}
	else if ( isset( $args['overflow'] ) && $args['overflow'] == 'hidden' ) {
		$classes[] = 'hidden';
	}
	$classes[] = 'resize';
	$str = implode( ' ', $classes );
