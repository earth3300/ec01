...
Description: Reworked menus to remove extra classes and provide cleaner HTML. Caches the menu to disk when the menu is updated. Saves to /theme/html/menu/menu.html. This can then be used by the theme as a cached page fragment.
Version: 2018.03.27
Author: wp.cbos.ca
...
function wp_bundle_cache_menu_on_save( $nav_id ){
	...
	if ( ! empty ( $str ) && defined ( 'SITE_MENU_PATH' ) ) {
		$result = file_put_contents( SITE_MENU_PATH . SITE_MENU_DIR . SITE_HTML_EXT,  $str );
	}
}
...
/**
 * Theme location may be "main-menu"
 * nav_menu_locations may return "primary"
 * These need to match.
 * If nav_menu_locations returns "primary"
 * The main menu name can be changed to "primary" in: Appearance:Menus:Menu Name.
 *
 */
